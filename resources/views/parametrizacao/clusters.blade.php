    ...
    <div class="side-app">
        ...
        <div class="page-header">
            <div class="page-leftheader">
                ...
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="#"><i class="fe fe-users mr-2 fs-14"></i>Tipos de cluster</a></li>
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <span class="font-weight-bold"><i class="fe fe-database mr-1"></i>Lista de clusters</span>
                    <div>
                        <a data-effect="effect-scale" data-toggle="modal" href="#modaldemo8" class="btn btn-info btn-sm modal-effect"><i class="fe fe-plus-circle mr-1"></i> Criar novo </a>
                    </div>
                </div>
                        ...
                        <table class="table  text-nowrap  table-striped table-hover table-bordered table-sm" id="example2">
                            <thead>
                                <tr>
                                    ...
                                    <th class="wd-15p border-bottom-0">Cluster</th>
                                    <th class="wd-15p border-bottom-0"></th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($clustersList as $items )
                                <tr style="transition: none !important; transform: none !important;">
                                    <td class="id">{{ $items->id_cluster}}</td>
                                    <td class="nome">{{$items->nome_cluster}}</td>
                                    <td>
                                        <div aria-label="Basic example" class="btn-group float-right" role="group">
                                            <a class="btn btn-primary btn-sm competenciaUpdate" data-toggle="modal" data-id="'.$items->id_cluster.'" data-target="#edit_competencia"> <i class="zmdi zmdi-edit"></i>&nbsp;Editar</a>
                                            {{-- <a href="{{ url('excluir/tipo/cluster/'.$items->id_cluster)}}" class="btn btn-secondary btn-sm" onclick="return confirm('Deseja excluir este tipo de cluster?')"><i class="zmdi zmdi-delete"></i>&nbsp;Excluir</a> --}}
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content modal-content-demo">
            <div class="modal-header">
                <h6 class="modal-title"> <i class="fe fe-plus-circle"></i> Cadastro de cluster</h6>
                <button aria-label="Close" class="close" data-dismiss="modal" type="button">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form action="{{route('form/tipo/cluster/save')}}" method="POST" id="validate">
                @csrf
                <div class="modal-body">
                    <div class="">
                        <div class="form-group">
                            <label for="nome" class="form-label">Tipo de cluster</label>
                            <input type="text" class="form-control" name="nome" id="nome" placeholder="Digite o tipo de cluster" onfocus="this.placeholder=''" onblur="this.placeholder='Digite o tipo de cluster'" required>
                        </div>
                    </div>
                </div>
...
<div class="modal" id="edit_competencia">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content modal-content-demo">
            ...
            <form action="{{route('form/tipo/cluster/update')}}" method="POST">
                @csrf
                <div class="modal-body">
    ...
    <div class="modal" id="delete_tecnico">
        <div class="modal-dialog modal-sm modal-dialog-centered" role="document">
            <div class="modal-content modal-content-demo">
                ...
                <form action="{{route('cluster/edit/delete')}}" method="POST">
                    @csrf
                    <input type="hidden" name="id" class="e_id" value="">
                    <div class="modal-body">
                        <p>Você deseja excluir esta cluster?</p>
                    </div>
