        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <span class="font-weight-bold"><i class="fe fe-database mr-1"></i>Lista de competências</span>
                    <div>
                        <a data-effect="effect-scale" data-toggle="modal" href="#modaldemo8" class="btn btn-info btn-sm modal-effect"><i class="fe fe-plus-circle mr-1"></i> Criar nova </a>
                    </div>
                </div>
                        ...
                        <table class="table  text-nowrap  table-striped table-hover table-bordered table-sm" id="example2">
                            <thead>
                                <tr>
                                    ...
                                    <th class="wd-15p border-bottom-0">Competência</th>
                                    <th class="wd-15p border-bottom-0">Descrição</th>
                                    <th class="wd-15p border-bottom-0"></th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($competenciasList as $items )

                                <tr style="transition: none !important; transform: none !important;">
                                    <td class="id small">{{ $items->id}}</td>
                                    <td class="nome small">{{ $items->nome_competencia}}</td>
                                    <td class="small descricao">{{ $items->descricao_competencia}}</td>
                                    <td>
                                        <div aria-label="Basic example" class="btn-group float-right" role="group">
                                            <a href="{{ url('editar/competencia/'.$items->id)}}" class="btn btn-primary btn-sm"> <i class="zmdi zmdi-edit"></i>&nbsp;Editar</a>
                                            <a href="{{ url('excluir/competencia/'.$items->id)}}" class="btn btn-secondary btn-sm" onclick="return confirm('Tem certeza que deseja excluir esta competência?')"> <i class="zmdi zmdi-delete"></i>&nbsp;Excluir</a>
...
<div class="modal" id="modaldemo8">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content modal-content-demo">
            ...
            <form action="{{route('form/competencia/save')}}" method="POST" id="validate">
                @csrf
                <div class="modal-body">
                    <div class="">
                        <div class="form-group">
                            <label for="nome" class="form-label">Competência</label>
                            <input type="text" class="form-control" name="nome" id="nome" placeholder="Digite a competência" onfocus="this.placeholder=''" onblur="this.placeholder='Digite a competência'" required>
                        </div>
                        <div class="form-group">
                            <label for="descricao" class="form-label">Descrição</label>
                            <textarea class="form-control mb-4" placeholder="Descrição da competência" rows="3" name="descricao" id="descricao" onfocus="this.placeholder=''" onblur="this.placeholder='Descrição da competência'" required></textarea>
    ...
    <div class="modal" id="delete_tecnico">
        <div class="modal-dialog modal-sm modal-dialog-centered" role="document">
            <div class="modal-content modal-content-demo">
                ...
                <form action="{{route('competencia/edit/delete')}}" method="POST">
                    @csrf
                    <input type="hidden" name="id" class="e_id" value="">
                    <div class="modal-body">
                        <p>Você deseja excluir esta competência?</p>
                    </div>
